...
class House < ApplicationRecord
  ...
  def better_seeds_query
    # The line below will not fire a seeds-query for each `Plant` since all the 
    # seeds will already have been fetched by the time `map` is called.
    plants.includes(:seeds).map(&:seeds)
  end
end